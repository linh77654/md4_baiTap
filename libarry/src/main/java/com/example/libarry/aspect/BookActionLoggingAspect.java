package com.example.libarry.aspect;
...
public class BookActionLoggingAspect {
    ...
    @Before("execution(* com.example.libarry.controller.BookController.*(..))")
    public void logVisitorCount() {
        visitorCount++;
        logger.info("Visitor count: {}");
    }

    @AfterReturning(value = "execution(* com.example.libarry.service.BookService.borrowBook(..))", returning = "borrowId")
    ...
    @AfterReturning("execution(* com.example.libarry.service.BookService.returnBook(..))")
    ...
    @AfterThrowing(pointcut = "execution(* com.example.libarry.service.BookService.borrowBook(..))", throwing = "ex")
    ...
    @AfterThrowing(pointcut = "execution(* com.example.libarry.service.BookService.returnBook(..))", throwing = "ex")
